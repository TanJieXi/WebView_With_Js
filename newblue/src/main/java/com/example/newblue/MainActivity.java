...
public class MainActivity extends AppCompatActivity implements ConnectBlueToothListener, DealDataListener {
    ...
    @OnClick({R.id.btn_startSao, R.id.btn_disconnet, R.id.btn_two, R.id.btn_open, R.id.btn_three, R.id.btn_bpm,R.id.btn_bmi,R.id.btn_bgm,R.id.btn_hgb,R.id.btn_glhgb,R.id.btn_bft,R.id.btn_bua})
    ...
    public void onFetch(String type, int code, JSONObject resultJson) {
        ...
        switch (type){
            case BlueConstants.BLUE_EQUIP_TEM://体温
                ...
                break;
            case BlueConstants.BLUE_EQUIP_OXI://血氧
                result = "j血氧： " + resultJson.optString("oxi") +"脉率：" + resultJson.optString("pul") + "次每分钟";
                setData(tv_text,result,200);
                break;
            case BlueConstants.BLUE_EQUIP_URA://尿机
            ...
            case BlueConstants.BLUE_EQUIP_BPM://血压计
                ...
                break;
            case BlueConstants.BLUE_EQUIP_BMI://体重体脂秤
                result = "j体重:" + resultJson.optString("weight")  + ",脂肪:" + resultJson.optString("fat")  + ",水分:" + resultJson.optString("water")
                        + ",肌肉:" + resultJson.optString("muscle")  + ",骨量:" + resultJson.optString("bone")  + ",卡路里:" + resultJson.optString("calorie")
                        + ",BMI:" +resultJson.optString("BMI")  + "Kg/㎡" + ",身高:" + resultJson.optString("height")  + "m";
                setData(tv_text,result,200);
                break;
            case BlueConstants.BLUE_EQUIP_BGM://血糖计
                result = "j血糖结果：" + resultJson.optString("bgm") + "单位：" + resultJson.optString("unit");
                setData(tv_text,result,200);
                break;
            case BlueConstants.BLUE_EQUIP_HGB://血红蛋白检测
                result = "j血红蛋白结果：" + resultJson.optString("hgb") + "单位：" + resultJson.optString("unit");
                setData(tv_text,result,200);
                break;
            case BlueConstants.BLUE_EQUIP_GLHGB://血红蛋白检测
                result = "j糖化血红蛋白结果：" + resultJson.optString("glHgb") + "单位：" + resultJson.optString("unit");
                setData(tv_text,result,200);
                break;
            case BlueConstants.BLUE_EQUIP_BFT: //血脂
