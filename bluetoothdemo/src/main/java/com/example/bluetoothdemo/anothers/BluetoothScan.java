...
//							.bytes2HexString(scanRecord)));
                    newdevice.setUuidHeadWords(Conver
                            .bytes2HexString(scanRecord));
                    ...
                    if (newdevice.Type.length() > 0) {
                        ...
                        for (int i = 0; i < Conver.LeDevices.size(); i++) {
                            if (newdevice.getUuidHeadWords().contains(
                                    Conver.LeDevices.get(i).getUuidHeadWords())) {
                                havdevice = true;
                                Conver.LeDevices.get(i).reFreshDateTime = new Date();
                                Conver.LeDevices.get(i).device = device;
                            }
